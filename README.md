...
Disabling CUDA Graph can expose launch overhead, but it cannot explain a multi-second layer-profile result by itself. In the observed comparison, FP32 summed layer time was 464.45 ms while INT8 was 2799.12 ms: INT8 was **6.03x slower**. Two INT8 profile entries covered at least 80% (at least 2239 ms), with the single largest entry around 1243 ms. This is a concentrated INT8 engine/tactic/fused-subgraph problem, not a general long-tail or CUDA Graph issue. The next action is to inspect the first two rows of the INT8 hot-layer table and their precision/type—not to re-enable CUDA Graph.
...
The original ONNX operator inventory contains no `QuantizeLinear` or `DequantizeLinear`, so it requires a calibration cache. The build accepts either `CALIBRATION_CACHE=/path/cache.file make int8` or an ONNX model whose explicit Q/DQ nodes pass `scripts/inspect_qdq_onnx.py`; the old `INT8_EXPLICIT_QDQ=1` trust bypass is no longer accepted. See `README_P2_EXPLICIT_QDQ.zh.md` for representative-data PTQ, accuracy evaluation, and profile acceptance. For production acceptance, prefer representative-data calibration; synthetic cache is meant for bring-up/debug scenarios. After rebuilding, regenerate the profile: reports include actual TensorRT output precision per layer and time grouped by FP32/FP16/INT8, making precision fallback and conversion-heavy hotspots visible.
