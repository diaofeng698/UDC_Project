...
struct CudaStream {
    ...
    ~CudaStream() { if (value != nullptr) cudaStreamDestroy(value); }
...
struct CudaEvent {
    ...
    ~CudaEvent() { if (value != nullptr) cudaEventDestroy(value); }
...
nvinfer1::Dims parseDims(std::string const& text) {
    ...
    if (dims.nbDims == 0) throw std::runtime_error("empty shape: " + text);
    return dims;
}
...
#endif
    default: throw std::runtime_error("unsupported TensorRT I/O data type");
    }
}
...
std::size_t volume(nvinfer1::Dims const& dims) {
    std::size_t result = 1;
    for (int i = 0; i < dims.nbDims; ++i) {
        if (dims.d[i] <= 0) throw std::runtime_error("unresolved dynamic shape: " + dimsString(dims));
        auto value = static_cast<std::size_t>(dims.d[i]);
        if (result > std::numeric_limits<std::size_t>::max() / value) throw std::overflow_error("tensor volume overflow");
        result *= value;
    }
    return result;
}
...
std::vector<char> readBinary(std::string const& path) {
    ...
    file.seekg(0);
    if (!file.read(data.data(), size)) throw std::runtime_error("cannot read engine: " + path);
    return data;
}
...
Stats calculate(std::vector<double> const& values) {
    ...
    double squared = 0.0;
    for (double value : values) squared += (value - stats.mean) * (value - stats.mean);
...
Options parseOptions(int argc, char** argv) {
    ...
    for (int index = 1; index < argc; ++index) {
        ...
        if (argument == "--engine") options.engine = requireValue("--engine");
        else if (argument == "--warmup") options.warmup = std::stoi(requireValue("--warmup"));
        else if (argument == "--iterations") options.iterations = std::stoi(requireValue("--iterations"));
        else if (argument == "--json") options.json = requireValue("--json");
        else if (argument == "--no-transfers") options.transfers = false;
        else if (argument == "--shape") {
            std::string specification = requireValue("--shape");
            auto equals = specification.find('=');
            if (equals == std::string::npos) throw std::runtime_error("--shape format is tensor=1x3xHxW");
    ...
    if (options.engine.empty()) throw std::runtime_error("--engine is required");
    if (options.warmup < 0 || options.iterations <= 0) throw std::runtime_error("warmup must be >=0 and iterations must be >0");
    return options;
}
...
int main(int argc, char** argv) {
    try {
        ...
        if (!engine) throw std::runtime_error("deserializeCudaEngine failed; verify TensorRT/GPU compatibility");
        ...
        for (auto const& [name, shape] : options.inputShapes) {
            ...
            if (!context->setInputShape(name.c_str(), shape)) throw std::runtime_error("setInputShape failed for " + name);
        }
        if (!context->allInputDimensionsSpecified()) throw std::runtime_error("dynamic input dimensions unresolved; provide --shape name=...");
        ...
        for (int index = 0; index < engine->getNbIOTensors(); ++index) {
            ...
            if (tensorName == nullptr) throw std::runtime_error("getIOTensorName returned null");
            Buffer buffer;
            ...
            if (!context->setTensorAddress(tensorName, buffer.device)) throw std::runtime_error("setTensorAddress failed for " + buffer.name);
        ...
        auto execute = [&](bool measure, std::vector<double>* h2d, std::vector<double>* inference,
            ...
            if (options.transfers) {
                for (Buffer const& buffer : buffers) if (buffer.mode == nvinfer1::TensorIOMode::kINPUT)
            ...
            if (!context->enqueueV3(stream.value)) throw std::runtime_error("enqueueV3 failed");
            ...
            if (options.transfers) {
                for (Buffer const& buffer : buffers) if (buffer.mode == nvinfer1::TensorIOMode::kOUTPUT)
        ...
        for (int index = 0; index < options.warmup; ++index) execute(false, nullptr, nullptr, nullptr, nullptr);
        std::vector<double> h2d, inference, d2h, endToEnd;
        h2d.reserve(options.iterations); inference.reserve(options.iterations);
        d2h.reserve(options.iterations); endToEnd.reserve(options.iterations);
        for (int index = 0; index < options.iterations; ++index) execute(true, &h2d, &inference, &d2h, &endToEnd);
        ...
        std::cout << "\nLatency statistics\n";
        if (options.transfers) printStats("H2D", h2dStats);
        printStats("Inference", inferenceStats);
        if (options.transfers) printStats("D2H", d2hStats);
        ...
        if (!options.json.empty()) {
            ...
            if (!output) throw std::runtime_error("cannot write JSON: " + options.json);
